...
class espn {
	...
	*/
	...
	public function getAtheleteNews($id){
		$res =  $this->espn_get($this->uriapi . $this->apiV . "/" . "sports/football/nfl/athletes" . "/" . $id . "/news/" . $this->apiR . $this->apiKey);
		if ($this->rawop)
			return $res;
		else
			return $this->streamline($res);
	...
	/* Not working yet
	public function getAtheleteInfo($id) {
		$res = $this->espn_get($this->uriapi . $this->apiV . "/" . "sports/football/nfl/athletes" . "/" . $id . $this->apiR . $this->apiKey);
		if ($this->rawop) return $res;
		$toreturn = array();
		for ($i=0; $i < count($res->{'sports'}[0]->{'leagues'}->{'athletes'}); i++) {
			$toreturn[$i]['id'] = $res->{'sports'}[0]->{'leagues'}[0]->{'athletes'}[$i]->{'id'};
			$toreturn[$i]['id'] = $res->{'sports'}[0]->{'leagues'}[0]->{'athletes'}[$i]->{'displayName'};
		}
		return $toreturn;
	} */
